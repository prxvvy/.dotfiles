...
int tlinelen(int y) {
    ...
    if (line[i - 1].mode & ATTR_WRAP) return i;
...
void draw(void) {
    ...
    if (TLINE(term.ocy)[term.ocx].mode & ATTR_WDUMMY) term.ocx--;
    if (TLINE(term.c.y)[cx].mode & ATTR_WDUMMY) cx--;
